(ns clojure-training-2024.lesson06
  (:require [medley.core :as m]
            ...
            [clojure-training-2024.lesson06-2 :as lesson06-2 :refer [i-do-something]]))
...
;; {:inventor "Doc Brown",
;;  :ns #namespace[clojure-training-2024.lesson06],
;;  :name foo-bar,
;;  :file "d:\\Visual Studio Code\\clojure-training-2024\\src\\clojure_training_2024\\lesson06.clj",
;;  :column 1,
;;  :line 17,
;;  :tag java.lang.Integer,
;;  :arglists ([x y]),
;;  :doc "Do some foo thang to them bar doohickies"}
...
;;  :name foo-2,
;;  :ns #namespace[clojure-training-2024.lesson06],
;;  :doc "The second foo is important, too!"}
...
#_(let [resource (atom {:some-resource :open})]
...
#_(declare bad-tree? parse-good-tree parse-tree)

#_(defn parse-tree [tree hint]
    (if (bad-tree? tree)
...
(try+
 (throw+ {:type ::kyaru :food 0})
 (catch [:type ::kyaru] {:keys [food]}
   (log/error "Kyaru is out of food: " food)))
...
#_(print (:body @response))

;;; Cheshire  --- a fast JSON encoder/decoder
#_(json/parse-string (:body @response))

;; And look how well these can play together:
#_(json/parse-string (:body @response) csk/->kebab-case-keyword)
...
(try+ (http/get "https://api.stripe.com/v1/charges" {:throw-entire-message? true})
  ;; Status 401 is request unauthorized
      (catch [:status 401] {:keys [request-time headers body]}
        [401 request-time headers])) ; and then body

#_(try (http/get "https://api.stripe.com/v1/charges" {:throw-entire-message? true})
       (catch Exception e
         e))
...
#_(ns-unmap *ns* 'speak)                  ; What to do when you get in trouble
...
#_(speak {:name "Sylvester" :type :cat})
...
(speak {:name "John Impact" :type :game})
...
(defn coin-value [machine]
  (reduce + (:coins machine)))

#_(ns-unmap *ns* 'handle)

(defmulti handle (fn [machine event]
...
(handle vending-machine {:type :coin :coin-value 20})
; {:state :accumulating-money, :coins [20]}
...
(def search-engines
  "The online search engines available to the FOOBAR software.
   ...
   The value should be a map, with key being the search-engine and
   with values consisting of the string
...
(clojure-training-2024.lesson06-2/i-do-something)

(lesson06-2/i-do-something)

;; won't compile because i-do-something already refers to something else
#_
(defn i-do-something []
  (println "I am the original"))
...
;;  :line 19,
;;  :column 1,
;;  :file "d:\\Visual Studio Code\\clojure-training-2024\\src\\clojure_training_2024\\lesson06_2.clj",
;;  :name foo,
;;  :ns #namespace[clojure-training-2024.lesson06-2]}
...
;; how it's actually compiling all of it is as:
;; clojure-training-2024.lesson06/foo
;; clojure-training-2024.lesson06-2/foo